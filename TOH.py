...
with model:
    ...
    model.goal_peg = spa.Buffer()
    model.focus_peg = spa.Buffer()
    model.target_peg = spa.Buffer()
    model.goal_final = spa.Buffer()
    model.largest = spa.Buffer()

    mdoel.set_focus = spa.Buffer()
    mdoel.set_goal = spa.Buffer()
    mdoel.set_goal_peg = spa.Buffer()
    ...
    bg_actions = [
        ...
        "(dot(focus, D2-D1-D0) + dot(goal, D2) - dot(goal_peg, target_peg))/np.sqrt(2) --> set_focus=D1",
        "(dot(focus, D2-D1-D0) + dot(goal, D2) + dot(goal_peg, target_peg))0.7/np.sqrt(3) --> set_focus=D1, set_goal=D1, goal_final=set_goal_peg",
        "(dot(focus, D1-D0) + dot(goal, D1) - dot(goal_peg, target_peg))/np.sqrt(2) --> set_focus=D0",
        "(dot(focus, D1-D0) + dot(goal, D1) + dot(goal_peg, target_peg))0.7/np.sqrt(3) --> set_focus=D0, set_goal=D0, goal_final=set_goal_peg",
        "(dot(focus, D0) + dot(goal_peg, target_peg))0.7/np.sqrt(2) --> set_focus=NONE",
        "(dot(focus, D0) + dot(goal, D0) - dot(goal_peg, target_peg))0.9/np.sqrt(3) --> set_focus=NONE, move_disk=D0, goal_final=set_goal_peg",
    ...
    if(disk_count > 3):
        bg_actions.append(
            ...
            "(dot(focus, D3-D2-D1-D0) + dot(goal, D3) - dot(goal_peg, target_peg))/np.sqrt(2) --> set_focus=D2",
            "(dot(focus, D3-D2-D1-D0) + dot(goal, D3) + dot(goal_peg, target_peg))0.7/np.sqrt(3) --> set_focus=D2, set_goal=D2, goal_final=set_goal_peg"
            )

    model.bg = spa.BasalGanglia(actions=tuple(bg_actions))
